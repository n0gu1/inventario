<x-admin-layout
    ...
    <x-wire-card>
        <form action="{{route('admin.customers.update', $customer)}}" method='POST' class="space-y-4">
            ...
            <x-wire-input label="Nombre" name="name" placeholder="Nombre del cliente" value="{{ old('name', $customer->name) }}" required />
            <x-wire-input label="Direccion" name="address" placeholder="Direccion del cliente" value="{{ old('address', $customer->address) }}" />
            <x-wire-input label="Correo electronico" name="email" type="email" placeholder="Correo electronico del cliente" value="{{ old('email', $customer->email) }}" />
